...
async function runOnTreadmill(timeLeft) {
  return new Promise((resolve, reject) => {
    if (timeLeft < 500) {
...
async function liftWeights(timeLeft) {
  return new Promise((resolve, reject) => {
    if (timeLeft < 2000) {
...
async function workout(totalTime) {
  try{
    let result1 = await stretch(totalTime)
    let result2 = await runOnTreadmill(result1)
    let result3 = await liftWeights(result2)
    console.log(`done working out with ${result3/1000} seconds left`)
